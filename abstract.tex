...
（2）针对化学资产重建问题，本文提出了一种基于隐式神经场的高保真化学物体重建算法，构建了从多视角图像输入到显式纹理模型输出的完整流程。针对透明物体表面提取不稳定的问题，本文采用基于绝对距离场局部极小值搜索的统一表面提取策略。针对小物体监督信号稀疏、训练收敛缓慢的问题，本文提出学习态增强策略，以提升有限训练预算下的几何重建质量。针对神经隐式场难以直接生成可用纹理资产的问题，本文进一步提出高分辨率纹理贴图生成方法，生成可直接用于仿真平台和渲染引擎的显式纹理模型。
...
Second, this dissertation proposes a high-fidelity reconstruction algorithm for chemical assets based on implicit neural fields. The method builds a complete pipeline from multi-view images to explicit textured models. For transparent objects, a unified surface extraction strategy based on local minima in the absolute distance field is adopted. For small objects, a learning state enhancement strategy is proposed to improve geometric reconstruction under limited training budgets. A high-resolution texture generation method is also developed to produce explicit textured assets for simulation platforms and rendering engines.

...
\caption{基于大语言模型的化学实验场景生成算法框架图。该框架以基于大语言模型的场景生成算法为核心，并进一步结合面向场景展示的相机规划模块与人机协同的场景进化算法，对初始生成场景进行高效评审与迭代优化。}
...
\begin{algorithm}[h]
\caption{LLM-driven Chemical Experiment Scene Generation}
...
\begin{figure}[!t]
  ...
  \caption{相机规划流程图。（a）三维场景，（b）二维平面图，（c）障碍物距离场，（d）二值化障碍物场（红色点为回环点），（e）相机轨迹（红色曲线），（f）蓝色箭头表示具有完整 6 自由度姿态的相机候选视角。（g）绿色箭头是最优的相机子路径}
\label{fig:camera_planning_flow}
\end{figure}


本章所提出的相机规划算法流程如图~\ref{fig:camera_planning_flow}和算法\ref{algo:camera_planning}所示。
...
\begin{algorithm}[!t]
\caption{Camera Planning for Scene Visualization}
...
\caption{交叉结果图。融合父代不同的设计模式，生成多样化的子代。}
...
\noindent\textbf{变异算子：}
变异算子的目标是在维持场景基本合理性的前提下，通过引入适度扰动来探索搜索空间中的潜在布局方案，生成形态各异且安全可用的实验场景，如图 \ref{fig:mutate_furniture} 所示。本章将变异过程定义为：
\begin{equation}
...
\caption{变异结果图。在场景整体设计风格不变的前提下，通过调整物体属性与空间关系，生成多样化的布局方案。}
...
\begin{algorithm}[!t]
\caption{LLM-driven Evolutionary Optimization}
\label{algo:llm_evo}
\begin{algorithmic}[1]
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{化学实验场景生成结果对比。(a) 不同大语言模型生成的化学实验场景平均 CLIP Score 对比。(b) GPT 与 Qwen 在三个阶段的关键物品覆盖率对比。其中，First Pass 表示未经安全补齐的初始物品清单，Safety+ 表示经过安全补齐后的理论物品清单，Post-Retrieval 表示经过资产检索与匹配后的实际物品清单。}
...
\begin{figure}[!t]
  ...
  \caption{相机规划结果图。左侧为场景俯视图，右侧上方为沿相机路径拍摄的关键帧，右侧下方为对应的相机轨迹。红线表示相机路径，绿色箭头表示相机位置与朝向。}
  \label{fig:camera_planning}
\end{figure}


图~\ref{fig:camera_planning} 展示了本章所提的相机规划算法在室内场景中的可视化效果。图中红色轨迹是相机规划的路径，绿色箭头是最优子序列的观测视角。观测视角能够较好地覆盖场景中的显著性物体，例如在图~\ref{fig:camera_planning:a}Kitchen 例子中，规划的视角能很好的捕获到饭桌、冰箱和椅子等显著性物体，能够有效帮助用户快速聚焦场景的核心区域。在图~\ref{fig:camera_planning:b}Living Room 例子中，规划的视角尽可能覆盖了场景中全部物体，能让用户对场景有完整的了解。
...
\begin{figure}[!t]
    ...
    \caption{传统随机算子与大语言模型驱动算子的 CLIP Score 对比结果。}

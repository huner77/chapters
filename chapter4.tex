...
本章以 NeuS 作为基础训练框架，将目标物体表示为符号距离场和颜色场的组合。其中，符号距离场用于刻画目标物体表面附近的连续几何结构，颜色场用于建模不同空间位置和观察方向下的外观信息。在训练过程中，网络根据相机参数从图像像素发射光线，并在归一化空间中进行采样，通过可微体渲染得到对应像素的预测颜色。随后，利用预测颜色与输入图像颜色之间的差异对网络进行优化，同时结合主体 Mask 约束和距离场正则项提升几何学习的稳定性。训练完成后，隐式几何场用于后续显式网格提取，隐式颜色场则用于纹理贴图生成阶段的颜色查询。

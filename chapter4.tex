...
近年来，以 NeRF 为代表的神经隐式重建方法为高保真三维建模提供了新的技术路径。与传统显式点云或网格重建方法相比，基于神经隐式场通过连续函数刻画空间中的几何和外观信息，能够在多视角图像监督下恢复较为细致的三维结构和视角相关颜色。其中，NeuS\cite{wang2021neus} 将符号距离场与体渲染相结合，能够在多视角 RGB 图像输入下恢复较为连续、平滑且具有较好几何精度的物体表面，因此适合作为化学实验资产重建的基础框架。
...
但直接将 NeuS 算法应用于化学实验资产仍然面临若干问题。首先，化学实验物体中存在大量小尺度目标，这类物体在图像中占据区域较小，随机采样得到的训练光线多数落在背景区域，导致训练初期几何收敛速度较慢。其次，NeuS 的输出本质上仍然是隐式场，而机器人仿真平台和渲染引擎通常需要显式三角网格、标准 UV 坐标和高分辨率纹理贴图，因而需要进一步完成从隐式表示到显式纹理资产的转换。最后，透明玻璃器具作为化学实验场景中的重要物体类型，其表面成像机理更为复杂，基于零等值面的表面提取方式在该类物体上可能存在表面缺失或结构不稳定等问题。
...
本章以 NeuS 作为基础训练框架，将目标物体表示为符号距离场和颜色场的组合。其中，符号距离场用于刻画目标物体表面附近的连续几何结构，颜色场用于建模不同空间位置和观察方向下的外观信息。网络训练阶段的核心目标是通过多视角图像监督，使隐式几何场与颜色场在同一优化过程中逐步收敛，从而得到能够支撑后续网格提取与纹理贴图生成的连续三维表示。

具体而言，训练损失主要由颜色重建损失、主体 Mask 约束损失和距离场正则项组成。颜色重建损失用于约束网络渲染结果与输入图像之间的一致性，使颜色场能够学习目标物体在不同视角下的外观变化。主体 Mask 约束损失用于限制有效重建区域，使网络更加关注目标物体所在空间，减少背景区域对隐式几何学习的干扰。距离场正则项则用于约束符号距离场满足局部几何连续性，使其梯度能够稳定地表征表面附近的几何变化方向。上述损失可以概括为
\begin{equation}
    \mathcal{L} =
...
\end{equation}
其中，$\mathcal{L}_{\mathrm{color}}$ 表示颜色重建损失，$\mathcal{L}_{\mathrm{mask}}$ 表示主体 Mask 约束损失，$\mathcal{L}_{\mathrm{eik}}$ 表示距离场正则项，$\lambda_{\mathrm{mask}}$ 和 $\lambda_{\mathrm{eik}}$ 分别表示对应损失项的权重系数。

在上述训练目标中，距离场正则项与后续学习态增强策略关系最为密切。由于 SDF 梯度 $\nabla f_{\theta}(\mathbf{x})$ 可以反映表面附近的局部几何方向，网络训练过程中几何场的稳定性将直接影响粗几何形成速度和后续细节学习效果。对于普通尺度物体，多视角图像中通常包含较充分的主体区域监督，网络能够较快从初始化状态收敛到目标物体的粗略几何。然而，对于化学实验中的小物体，主体像素占比较低，有效监督信号稀疏，网络在训练初期更容易长时间停留在过渡阶段。因此，本章在基础训练损失之外进一步引入学习态增强策略，以提高训练早期 SDF 几何场的收敛效率。

在化学实验资产重建中，存在不少小尺度物体，它们在图像中仅占据较少像素，例如药匙、橡胶塞、夹具和小型连接件等。对于这类目标，随机采样得到的训练光线中，大量光线落在背景区域或自由空间区域，真正穿过物体表面及轮廓附近的有效光线比例较低，导致训练初期的几何收敛速度较慢。如图~\ref{fig:small_object_pixels} 所示，小物体的有效监督区域较小，网络更容易长时间停留在粗几何尚未稳定的阶段。
...
针对上述问题，本章在网络训练阶段引入学习态增强策略。该策略的核心思想是根据训练进度动态调节射线方向与 SDF 梯度方向之间的约束强度，使网络在训练初期能够更快形成稳定的粗几何，并在训练后期逐步转向精细表面优化。设当前采样点为 $\mathbf{x}$，对应射线方向为 $\mathbf{d}$，SDF 梯度为 $\nabla f_{\theta}(\mathbf{x})$。首先定义射线方向与局部 SDF 梯度之间的方向一致性量
\begin{equation}
    \cos_t =
    \mathbf{d}\cdot \nabla f_{\theta}(\mathbf{x}).
\end{equation}
其中，$\nabla f_{\theta}(\mathbf{x})$ 可以看作隐式表面在局部位置的法向方向，因此 $\cos_t$ 反映了当前采样方向与局部几何变化趋势之间的关系。

为了使方向约束适应不同训练阶段，本章进一步引入训练进度因子 $\cos_a$
\begin{equation}
\cos_{a}= 
\begin{cases}
...
\end{cases}
\end{equation}
其中，$\text{cur\_step}$ 表示当前训练步数，$\text{cos\_step\_end}$ 表示阶段切换的控制参数。训练初期 $\cos_a$ 较小，网络采用更宽松的方向引导以促进粗几何快速形成。随着训练推进，$\cos_a$ 逐渐增大并趋近于 $1$，方向约束逐步增强，使网络更加关注表面定位精度与几何细节刻画。

在此基础上，定义自适应采样因子
\begin{equation}
    \cos_i =
    (1-\cos_a)\cos_i^{\mathrm{soft}}
    + \cos_a \cos_i^{\mathrm{hard}},
\end{equation}
其中，$\cos_i^{\mathrm{soft}}$ 表示宽松模式下的方向项，$\cos_i^{\mathrm{hard}}$ 表示严格模式下的方向项。训练初期，$\cos_i$ 更接近宽松模式，有助于缓解小物体监督稀疏带来的粗几何收敛缓慢问题。训练后期，$\cos_i$ 更接近严格模式，有助于提升表面位置估计的准确性。

最后，将自适应采样因子用于相邻采样区间两端的 SDF 估计
\begin{equation}
\begin{cases}
\mathrm{SDF}_{\mathrm{next}} =
\mathrm{SDF} + \cos_i D,\\
\mathrm{SDF}_{\mathrm{prev}} =
\mathrm{SDF} - \cos_i D,
\end{cases}
\end{equation}
其中，$D$ 表示当前采样区间长度。该估计方式能够根据当前训练阶段和局部几何状态调节 SDF 变化幅度。在几何趋势较明确时，网络可以更快完成表面位置收敛。在几何尚不稳定时，较为保守的更新方式可以避免过大的位置偏移。通过这种方式，学习态增强策略能够缩短小物体训练中的过渡阶段，使网络更早进入细节学习阶段，并为后续网格提取和纹理贴图生成提供更稳定的隐式场。

综上，网络训练阶段首先利用多视角图像、相机参数和主体 Mask 对 NeuS 的几何场与颜色场进行联合优化，得到连续的 SDF 表示和隐式颜色表示。在此基础上，针对化学实验小物体像素占比较低、监督信号稀疏的问题，进一步引入学习态增强策略，提高训练初期粗几何形成效率。训练完成后，得到的 SDF 场可用于后续显式网格提取，颜色场则为纹理贴图生成提供颜色查询基础，从而衔接本章 pipeline 中的网格提取和纹理贴图生成模块。

\subsection{面向透明物体的几何表面提取}
\ensubsection{Geometry Surface Extraction for Transparent Objects}
\label{sec:ch4_transparent_object}

由于光线透射与复杂折射的影响，透明物体内部通常缺乏可稳定跟踪的纹理特征，因此透明物体重建一直是三维重建领域中的难点之一。Ichnowski等人的工作\cite{ichnowski2022dexnerf} 表明，NeRF 由于具备视点相关的外观建模能力，能够在训练过程中学习与透明性相关的几何信息，并恢复出质量较高的深度结果。

在基于 NeuS 的三维重建中，网络通过体渲染对隐式距离场进行优化，并通常将该距离场的零等值面视为目标物体表面，再借助 Marching Cubes 提取显式网格。对于不透明物体，这一策略通常是有效的，因为 NeuS 构造的体渲染权重在局部满足无偏性，权重峰值能够与零等值面较好对齐。然而，这一结论依赖于表面满足不透明假设；当重建对象扩展到透明物体时，零等值面与真实表面之间的对应关系不再总是成立。

这是因为透明物体的成像机理与不透明物体存在明显差异。对于透明区域，光线在传播过程中会受到透射特性的影响，使得体渲染权重的最大值不一定落在距离场的零交叉位置。Zhang等人的工作\cite{zhang2024alphaneus} 表明，在 NeuS 学得的距离场中，当表面不透明度较低时，最大渲染权重更可能对应于距离场中的非负局部极小值；而当表面不透明度较高时，最大渲染权重又会重新与零等值面接近对齐。因此，在透明与不透明区域共存的场景中，NeuS 学得的距离场不再是单一的标准 SDF，而更接近一种混合的 SDF/UDF 场。

在这种情况下，若仍直接使用零等值面作为统一的表面提取条件，则只能较稳定地恢复不透明区域，而透明表面由于往往对应正的局部极小值，通常不会出现在零等值面上，从而导致显式网格出现缺失或空洞。若改用固定的非零阈值进行等值面提取，虽然有时能够包络透明外壳，但由于透明区域局部极小值的大小并不固定，且会随空间位置变化，因此单一阈值难以同时匹配所有真实表面，提取结果往往只是近似包络，而非与渲染几何一致的目标表面。

基于上述分析，本章不再将零等值面简单视为唯一的目标表面，而是将与最大渲染权重对齐的位置统一定义为无偏表面。按照这一标准，不透明区域的目标表面仍可表示为距离场的零等值面，而透明区域的目标表面则对应距离场中的非负局部极小值。为在同一框架下统一提取这两类表面，本章进一步对 NeuS 学得的距离场 $f$ 取绝对值，构造绝对距离场
\begin{equation}
f^{a}(\mathbf{x}) = |f(\mathbf{x})|.
\end{equation}
经过这一变换后，原本对应透明表面的非负局部极小值以及原本对应不透明表面的零等值面，都会统一转化为 $f^{a}$ 中的局部极小值，从而将原始的混合 SDF/UDF 场转化为更适合局部极小值搜索的场表示。
...
在具体实现上，采用“先包围、后投影”的表面提取策略。首先，在原始距离场 $f$ 上选取一个用户指定的非零等值阈值 $r$，并通过 Marching Cubes 提取初始包围网格
\begin{equation}
M = \mathrm{MC}\bigl(f(\mathbf{x}),\, r\bigr),
\end{equation}
其中，$r$ 需要大于目标表面对应的局部极小值，以保证初始网格 $M$ 作为外层包围，能够完整覆盖待恢复的真实无偏表面。该网格并非最终结果，而是为后续投影优化提供一个拓扑连续的初始曲面。

在得到初始包围网格后，本章参考 Hou 等人的工作\cite{Hou2023DCUDF} 的局部极小值投影思想，将网格顶点及面心逐步收缩到绝对距离场 $f^{a}$ 的局部极小值位置。记初始网格的顶点集合为 $V$，面片连接关系为 $F$，由当前网格确定的三角面重心集合记为 $C(V,F)$。第一阶段采用粗投影策略，其优化目标写为
\begin{equation}
\min_{V}
\sum_{v_i \in V} f^{a}(v_i)
+\sum_{c_i \in C(V,F)} f^{a}(c_i)
+\lambda_1 \sum_{v_i \in V} w(v_i)\left\|\Delta v_i\right\|^2,
\label{eq:ch4_coarse_projection}
\end{equation}
其中，$\Delta v_i$ 为顶点 $v_i$ 的离散 Laplace 项，$w(v_i)$ 为与邻接三角面面积相关的权重。该阶段通过联合约束顶点与面心向绝对距离场的局部极小值收缩，并利用 Laplacian 正则保持优化过程的平滑性与稳定性，从而驱动初始包围网格整体逼近目标表面。

在粗投影结束后，记第一阶段结束时的三角面重心集合为 $C^{(1)}$，对应面法向集合为 $N^{(1)}=\{n_i^{(1)}\}$。在此基础上，第二阶段进一步引入法向约束进行细化，其优化目标为
\begin{equation}
\min_{V}
\sum_{v_i \in V} f^{a}(v_i)
+\sum_{c_i \in C(V,F)} f^{a}(c_i)
+\lambda_2 \sum_{c_i \in C(V,F)}
\left\|
\bigl(c_i-c_i^{(1)}\bigr)\times n_i^{(1)}
\right\|,
\label{eq:ch4_refine_projection}
\end{equation}
其中，$c_i^{(1)}$ 表示第一阶段结束时第 $i$ 个三角面的重心位置，$n_i^{(1)}$ 表示对应三角面的法向。该阶段通过约束面心主要沿参考法向方向移动，抑制切向漂移，从而进一步提高最终表面与真实无偏表面的贴合精度，并减少局部几何伪影。


算法~\ref{algo:transparent_surface_extraction} 给出了上述统一表面提取过程的完整流程。通过“初始包围网格提取—粗投影收缩—法向约束细化”三个阶段，初始包围网格会逐步收缩到绝对距离场的局部极小值处，从而实现对透明表面与不透明表面的统一恢复。



\subsection{基于学习态增强的小物体重建加速策略}
\ensubsection{Strategy for Ac
celerating Small Object Reconstruction}
\label{sec:adaptive_gradient}
...
\begin{figure}[!t]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \caption{}
    \end{subfigure}
    \caption{小物体示意图。小物体仅覆盖图像中的极小一部分区域，导致有效监督信号较为稀疏。}
    \label{fig:small_object_pixels}
\end{figure}

在小物体重建任务中，由于目标区域在输入图像中通常仅占据较少像素，如图~\ref{fig:small_object_pixels} 所示，因此随机采样得到的训练光线大多落在背景或自由空间区域，真正穿过物体表面及轮廓附近的有效光线比例较低，从而抑制了隐式几何表面的收敛效率。

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/ch4/NeuS_three_tai.png}
\caption{NeuS 训练过程中的几何演化图。训练通常遵循从初始单位球（初始态）到粗略几何（过渡态），再到精细表面刻画（学习态）的演化路径。}
\label{fig:neus_three_stage}
\end{figure}
这一问题在基于 NeuS 的重建过程中表现得尤为明显。如图~\ref{fig:neus_three_stage} 所示，基于 NeuS 的神经隐式表面重建通常经历三个关键阶段：首先，通过初始化将场景表示为单位球。随后，在过渡态中，单位球逐步坍缩并收敛为与目标物体形状接近的粗略几何。最后，在学习态下对高频几何细节进行细化。对于小物体而言，由于训练初期能够提供有效几何信息的光线数量有限，网络往往需要更长时间才能完成从初始单位球到粗略物体形状的收敛过程，因此更容易在过渡态中停留较久。

与此同时，在神经辐射场类算法中，对于每个物体都需要专门训练一个网络，这一过程通常涉及高达 30 万次的迭代优化，在单张主流显卡上往往需耗费数十小时的计算时间~\cite{wang2021neus}。若网络因监督信号稀疏而长期停留在过渡态，那么大量迭代步数就会被消耗在几何表面的初步构建上，真正分配给后期细节学习的优化预算将明显减少。这不仅降低了整体训练效率，也会直接限制最终的重建精度。

基于上述分析，本章提出一种学习态增强策略。该策略旨在针对小物体训练早期有效监督不足的问题，通过动态调节采样方向与有符号距离函数梯度方向的权重，引导网络更快完成初始形状收敛，缩短过渡态持续时间，从而尽早进入面向细节优化的学习态，并为高保真几何重建预留更充分的有效迭代空间。



具体而言，设当前采样点为 $\mathbf{x}$，对应射线方向为 $\mathbf{d}$，有符号距离函数（Signed Distance Function, SDF）记为 $f(\mathbf{x})$。为了刻画当前射线传播方向与局部几何法向之间的关系，首先定义射线方向与 SDF 梯度之间的方向一致性量 $\cos_t$：
\begin{equation}
\cos_t = \mathbf{d} \cdot \nabla f(\mathbf{x}),
\end{equation}
...
\subsection{基于 NeuS 的高分辨率纹理资产生成算法}
\ensubsection{High-Resolution Texture Asset Generation Algorithm Based on NeuS}
...
针对这一问题，本章构建了一条从 NeuS 神经隐式场到显式纹理资产的自动化生成流程，其核心思路如图~\ref{fig:xatlas_stage_pk}和流程\ref{algo:neus_texture_generation_en} 所示。
...
首先需要对重建得到的显式网格进行几何优化与色彩提取。针对训练完成的 NeuS 网络，本章采用第~\ref{sec:ch4_transparent_object} 节所提出的算法提取显式网格$M$，并进一步采用 Loop 细分算法（Loop Subdivision）\cite{loop1987smooth} 进行拓扑细化。该算法通过在每条边上插入新顶点并重计算原始顶点位置，将每个三角形面片分裂为四个子三角形。这一操作能够有效缓解局部面片分布不均的问题，为后续基于顶点的颜色采样提供更为均匀且密集的离散点集。
...
在完成顶点颜色提取后，需要进一步建立三维表面与二维纹理空间之间的映射关系，即生成纹理坐标（UV）。为此，本章引入 Xatlas~\cite{young_xatlas}开源纹理生成库对显式网格进行自动参数化处理，其核心过程如图~\ref{fig:xatlas_uv} 所示。该算法首先依据网格的曲率分布与局部几何特征，对复杂表面进行自动化分块（Charts），再通过基于几何代价的切割与布局优化，将这些局部表面展开并紧凑排列到统一的二维纹理坐标空间中。该过程为后续将三维表面的颜色信息映射到二维纹理图像奠定了几何基础。
...
在数据集方面，透明物体重建实验采用 Ichnowski 等人的工作\cite{ichnowski2022dexnerf}公开的透明物体数据和 Synthetic Blender Dataset\cite{mildenhall2020nerf} 验证算法在透明材质场景下的几何恢复能力。小物体重建实验采用 Jittor 平台公开的神经隐式表示三维重建数据\cite{jittor2023implicit3dchallenge}。纹理贴图生成实验采用第 26 届全国计算机辅助设计与图形学学术会议"高精度三维重建挑战赛"提供的公开数据集\cite{cadcg2023reconchallenge}，该数据集包含 21 个具有高分辨率输入图像及对应相机参数的物体，适合用于评估三维资产的重建效果。

在对比设置方面，由于本章的所有工作均建立在原始 NeuS 框架之上，因此本章统一采用原始 NeuS 作为对比基线算法，验证各改进模块的有效性。对于透明物体实验，基线算法直接采用零等值面提取方式生成显式网格，本章算法则采用基于绝对距离场局部极小值搜索的统一表面提取策略，以比较两者在透明区域表面恢复能力上的差异。对于小物体实验，基线算法采用原始 NeuS 的训练机制，本章算法在此基础上引入学习态增强策略，以比较两种算法在相同训练预算下的收敛速度、轮廓成形过程以及最终重建质量。对于纹理重建实验，本章将同一组顶点带有颜色的显式网格分别输入 MeshLab \cite{cignoni2008meshlab} 与本章算法进行纹理生成，MeshLab 采用其内置的Texture from Vertex Color功能完成纹理展开与贴图生成。本章算法则采用 Xatlas 进行 UV 参数化，并结合光栅化烘焙、纹理贴图修复，以比较不同纹理生成方案在表面连续性与重渲染保真度上的表现。
...
\subsection{透明物体实验分析}
\ensubsection{Reconstruction Results and Analysis on Transparent Objects}
\begin{figure}[!t]
    \centering

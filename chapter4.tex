...
本章围绕化学实验资产类型多样且现有数据集覆盖不足的问题，首先提出了化学实验物体分类体系图，将实验室物体从光学属性、拓扑属性、尺度属性和纹理属性四个维度进行分类，并据此分析不同类别物体带来的重建挑战。在此基础上，提出了一种基于隐式神经场 NeuS 的高保真化学资产重建算法，该算法能够有效应对各个类别物体所带来的重建挑战。
...
在面向化学实验的具身智能任务场景中，机器人不仅需要理解实验步骤和操作语义，还需要获得实验物体准确的三维几何与纹理信息，从而支撑后续的抓取、放置、倾倒、搅拌、仿真和场景交互。但是当前通用三维物体数据集\cite{mildenhall2020nerf,jensen2014large,knapitsch2017tanks,yao2020blendedmvs}以日常物体为主，缺少对化学实验室资产的系统覆盖。对于化学实验物体而言，在几何结构、光学成像、尺度分布和表面纹理上存在显著差异，因此，有必要从三维重建视角对化学实验物体进行属性化分类，并针对其特性设计高保真重建算法。


\begin{figure}[!t]
  \centering
  \includegraphics[width=\textwidth]{figures/ch4/lab_object.png}
  \caption{化学实验物体分类体系。}
  \label{fig:chemical_object_taxonomy}
\end{figure}


基于上述考虑，本章首先参考工作\cite{verbin2022refnerf、zhang2024alphaneus、sajjan2019cleargrasp、kazhdan2006poisson、fang2022transcg、wu2025alphasurf、hollein2023text2room、schonberger2016sfm、xu2020marmvs}提供的分类与重建挑战，构建如图~\ref{fig:chemical_object_taxonomy} 所示的化学实验物体分类体系。该分类体系是面向三维重建问题的属性分析框架。同一物体可以同时具有多个属性，例如带标签烧杯既属于透明物体，又具有文字纹理和开口实体结构。具体而言，本章从光学属性、拓扑属性、尺度属性和纹理属性四个维度对化学实验物体进行划分。光学属性反映物体在多视角成像中的反射、透射和散射特性，主要包括不透明物体和透明物体，其中不透明物体可进一步表现为漫反射物体和高光反射物体，透明物体则可进一步表现为薄壁透明体、厚壁透明体、半透明体和多界面透明体。拓扑属性反映物体表面连接关系和几何结构形式，主要包括开口实体、闭合实体、薄壁结构和多连通结构。尺度属性反映物体在图像中的相对占比，主要包括常规尺度物体和小尺度物体。纹理属性反映物体表面外观信息的分布特点，主要包括文字纹理、重复纹理、弱纹理和高频纹理。
...
在化学实验资产重建中，存在不少小物体，它们在图像中仅占据较少像素，例如药匙、橡胶塞、夹具和小型连接件等。对于这类目标，随机采样得到的训练光线中，大量光线落在背景区域或自由空间区域，真正穿过物体表面及轮廓附近的有效光线比例较低，导致训练初期的几何收敛速度较慢。如图~\ref{fig:small_object_pixels} 所示，小物体仅覆盖图像中的较小区域，导致有效监督信号较为稀疏，网络更容易长时间停留在粗几何尚未稳定的阶段。
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/ch4/small_object.png}
    \caption{小物体示意图。}
    \label{fig:small_object_pixels}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/ch4/NeuS_three_tai.png}
\caption{NeuS 训练过程中的几何演化图。}
...
\subsection{几何重建实验分析}
\ensubsection{Geometric Reconstruction Experimental Analysis}

本节从不透明物体和透明物体两个方面分析本文算法的几何重建效果。其中，不透明物体实验主要关注化学实验资产中小物体的重建问题，用于验证学习态增强策略对训练收敛过程和几何细节恢复的影响。透明物体实验主要用于验证 NeuS 神经隐式场在透明物体上的几何建模能力。


\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/ch4/Neus_Small_Object_PK.png}
    \caption{小物体重建阶段可视化对比图。}
    \label{fig:neus_small_object_pk}
\end{figure}

如图~\ref{fig:neus_small_object_pk} 所示，在同等计算预算下，本章算法得到的几何重建结果显著优于原始 NeuS。小物体由于在图像空间占据的像素比例较低，在训练初期往往面临监督信号稀疏的问题，导致模型长时间停留在低频特征的学习上。针对这一挑战，本章引入了采样方向与 SDF 梯度方向的动态调节机制，加速网络从初始态向过渡态（10k~50k 次迭代）的跨越。这种策略使得网络能够快速建立粗几何轮廓，从而在后续的学习态（50k~200k 次迭代）中，将更多的计算资源集中于高频细节的精细刻画。
...
\begin{table}[!t]
\centering
\caption{透明物体重建的 Chamfer Distance$\downarrow$对比表（单位：mm）}
...
综合不透明物体与透明物体的实验结果可以看出，本章算法在几何重建层面分别针对两类典型问题进行了改进。对于不透明小物体，学习态增强策略能够缓解训练初期监督信号稀疏导致的长时间过渡问题，使模型更早进入稳定的几何细化阶段，从而为后续高频细节学习预留更多有效迭代空间。对于透明物体，基于绝对距离场局部极小值搜索的表面提取策略能够缓解零交叉位置与真实表面不一致造成的表面缺失问题，在保持整体结构完整性的同时更稳定地恢复物体几何边界。
...
\end{figure}
本节纹理贴图生成实验主要用于验证从 NeuS 隐式神经场到显式纹理资产转换流程的有效性，因此选取具有复杂表面纹理和高频外观细节的物体进行测试。该组实验重点评估复杂纹理情况下本文算法的纹理生成效果，而非限定于化学实验物体。

图~\ref{fig:texture_example} 展示了本章所提的高保真资产重建算法的重建效果，图中从左到右依次为输入图像、重建得到的三维资产、对应几何模型和纹理贴图。结果表明，本章算法能够将输入图像中的物体重建为较精细的几何模型和纹理贴图。对于苦瓜表面的凹凸细节和牛角包的烘焙纹理特征，算法均能够较准确地刻画。对于玉米的颗粒重复结构，生成的纹理贴图也较好地保留了原始图像中的外观特征，未出现明显的错乱与遗漏。
...
\begin{figure}[!t]
  ...
  \caption{MeshLab 与本章算法生成的纹理贴图对比图。}
...
\begin{table}[!t]
\centering
\caption{纹理贴图生成算法的 PSNR$\uparrow$ 对比表（单位：dB）}
...
尽管本章算法在几何重建和纹理贴图生成方面取得了较好的实验效果，但仍然存在一些局限性，有待在后续工作中进一步完善。

(1) 在几何重建方面，对于不透明小物体，学习态增强策略能够在一定程度上加速训练初期的几何收敛，但并未从根本上改变 NeuS 训练时间较长、优化过程依赖多视角监督的特点。相较之下，近年来以 Instant-NGP 为代表的高效神经表示算法\cite{muller2022instantngp,Li_2023_CVPR,Wang_2023_ICCV} ，通过多分辨率哈希编码等机制，显著提升了神经场的训练与查询效率，在保持较强表达能力的同时大幅缩短了优化时间。相比这类高效表示，本章算法仍然建立在较为传统的隐式场优化范式之上，因此在大规模数据处理、快速重建和交互式建模等应用场景中仍存在一定限制。对于透明物体，本章采用基于绝对距离场局部极小值搜索的表面提取策略，以缓解零等值面与真实表面不一致带来的表面缺失问题，但该方法仍未显式建模透射、折射和高光反射等物理光学过程。在复杂光照、强反射或多层透明结构下，隐式场学习到的几何边界仍可能出现波动，自动法向估计也可能带来局部法向翻转和明暗异常。因此，未来可以进一步结合高效神经表示、物理光学建模或多框架融合方法，以提升化学实验物体重建质量。
...
实验结果表明，本章算法在几何与纹理两个层面均取得了较好的效果。在透明物体重建中，所提出的表面提取策略能够更完整地恢复器皿外轮廓。在小物体重建中，学习态增强策略能够缩短过渡阶段并提升边缘与细节恢复能力。在纹理生成中，本章算法相比 MeshLab 等通用工具链能够有效减少纹理接缝、空洞与边缘锯齿，获得更加连续和平滑的表面外观。综上，本章提出的算法能够有效提升化学实验资产三维重建的保真度与工程可用性，为后续面向具身智能训练的高质量化学实验场景构建奠定了三维资产基础。

...
在数据集选择方面，目前公开数据集中缺少面向化学实验资产重建的数据集。理想的化学资产重建数据集需要覆盖常规实验器材、实验小物体与透明玻璃器具，并提供多视角 RGB 图像、相机参数、主体掩膜以及真值几何模型。然而，现有广泛应用于 NeRF 三维重建任务的公开数据集\cite{mildenhall2020nerf,jensen2014large,knapitsch2017tanks,yao2020blendedmvs}侧重于日常物体，难以完整覆盖上述需求。为了保证实验结果的可复现性与可比较性，本文选取具有公开数据、相机参数和较稳定评价条件的通用数据集进行实验验证，并结合化学实验资产重建中的典型问题对结果进行分析。

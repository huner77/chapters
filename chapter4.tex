...
在面向化学实验的具身智能任务场景中，高质量三维资产是连接场景生成、机器人仿真与任务执行的重要基础。机器人在执行化学实验任务时，不仅需要识别实验物体的类别，还需要获得其准确的三维几何、空间尺度与纹理外观信息，从而支撑后续的抓取、放置、倾倒、搅拌、碰撞检测与视觉感知等操作。相比于一般室内场景中的家具和日用物体，化学实验场景中的物体往往具有更强的功能属性和操作属性，其几何形态、材质外观和尺寸尺度都会直接影响仿真环境的真实性与机器人任务执行的可靠性。因此，构建面向化学实验场景的高保真三维资产，是实现化学实验具身智能系统的重要前提。

然而，现有开源三维资产库主要面向通用室内场景或日常生活物体，难以充分覆盖化学实验中常见的专业仪器与实验部件。即使能够获得部分通用三维模型，其几何精度、纹理质量和物理尺度也未必满足化学实验仿真与机器人操作的需求。对于化学实验场景而言，烧杯、试管、量筒、冷凝管、滴定管、铁架台、药匙、橡胶塞等物体都可能参与实验流程，且不同物体在材质、尺度和结构复杂度上差异明显。为了更清晰地分析化学实验物体对三维重建算法提出的需求，本文从材质可见性、几何结构和尺度特征三个角度对化学实验物体进行分类，如图~\ref{fig:chemical_object_taxonomy} 所示。

\begin{figure}[!t]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/ch4/chemical_object_taxonomy}
  \caption{面向三维重建的化学实验物体分类体系。}
  \label{fig:chemical_object_taxonomy}
\end{figure}

由图~\ref{fig:chemical_object_taxonomy} 可见，化学实验物体可以首先划分为透明物体与不透明物体。透明物体主要包括化学实验中常见的玻璃器具，其成像过程受到透射、折射和反射等因素影响，难以满足传统多视角重建方法中常用的理想表面假设。根据几何结构差异，透明物体可以进一步分为闭合型透明物体与薄壳型透明物体。闭合型透明物体通常具有相对完整的实体边界，例如玻璃棒、玻璃塞、实心玻璃部件等，其几何结构相对连续。薄壳型透明物体则主要包括烧杯、试管、量筒、培养皿、冷凝管、滴定管等玻璃器具，这类物体通常具有内外两层相邻表面，且壁厚较薄，容易在多视角重建过程中产生表面混叠、边界缺失或内部伪影等问题。

对于不透明物体，本文进一步将其划分为常见日用物体、常规实验器材和化学实验小物体。常见日用物体包括剪刀、夹子、托盘、纸盒、手套等辅助物体，这类物体在材质和纹理上通常较为稳定，适合采用常规多视角重建流程进行处理。常规实验器材包括铁架台、烧杯夹、酒精灯、电子天平、搅拌器等实验设备，其几何结构相对明确，但对尺度一致性和纹理表达具有较高要求。化学实验小物体包括瓶盖、药匙、磁力搅拌子、橡胶塞、连接头、试管夹等部件，这类物体虽然材质通常不复杂，但由于在输入图像中占据像素比例较低，容易造成有效监督信号稀疏，从而影响神经隐式场训练的收敛效率。由此可见，面向化学实验的三维资产重建不仅需要关注透明玻璃器具等特殊材质物体，还需要处理小尺度实验部件和常规不透明器材的高质量建模问题。

近年来，以 NeRF 为代表的神经隐式重建方法为高保真三维建模提供了新的技术路径。与传统点云、体素和三角网格等显式表示不同，神经隐式场通过连续函数刻画空间中的几何和外观信息，能够在多视角图像监督下恢复较为细致的三维结构和视角相关颜色。进一步地，NeuS\cite{wang2021neus} 将有符号距离函数与体渲染过程相结合，利用 SDF 的零等值面表示目标表面，在多视角表面重建任务中具有较好的几何精度和表面连续性。因此，NeuS 适合作为本文化学资产重建的基础框架，用于建立从多视角图像输入到隐式几何表示，再到显式三维资产输出的重建流程。

尽管 NeuS 在多视角物体重建中具有较好的基础能力，但直接将其应用于化学实验资产仍然面临若干问题。首先，化学实验物体中存在大量小尺度目标，这类物体在图像中占据区域较小，随机采样得到的训练光线多数落在背景区域，导致训练初期几何收敛速度较慢。其次，NeuS 的输出本质上仍然是隐式场，而机器人仿真平台和渲染引擎通常需要显式三角网格、标准 UV 坐标和高分辨率纹理贴图，因而需要进一步完成从隐式表示到显式纹理资产的转换。最后，透明玻璃器具作为化学实验场景中的重要物体类型，其表面成像机理更为复杂，标准 NeuS 中基于零等值面的表面提取方式在该类物体上可能存在表面缺失或结构不稳定等问题，因此有必要在实验分析中对其进行专门讨论。

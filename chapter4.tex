...
本章围绕不同类型化学资产统一重建需求，提出了一种基于隐式神经场 NeuS 的高保真化学资产重建算法。该算法利用多视角 RGB 图像恢复物体几何，并构建了一个包含数据预处理、隐式场训练、网格提取与优化和纹理贴图生成的完整重建流程。
...
在面向化学实验的具身智能任务场景中，机器人不仅需要理解实验步骤和操作语义，还需要获得实验物体准确的三维几何与纹理信息，从而支撑后续的抓取、放置、倾倒、搅拌、仿真和场景交互。但是当前通用三维物体数据集\cite{mildenhall2020nerf,jensen2014large,knapitsch2017tanks,yao2020blendedmvs}以日常物体为主，缺少对化学实验室资产的系统覆盖。对于化学实验物体而言，在几何结构、光学成像、尺度分布和表面纹理上存在显著差异，因此，有必要从三维重建视角对化学实验物体进行属性化分类，并针对其特性设计高保真重建算法，以满足化学实验仿真和具身智能训练的需求。

基于上述考虑，本章首先构建如图~\ref{fig:chemical_object_taxonomy} 所示的化学实验物体分类体系。该分类体系是面向三维重建问题的属性分析框架。同一物体可以同时具有多个属性，例如带标签烧杯既属于透明物体，又具有文字纹理和开口实体结构。具体而言，本章从光学属性、拓扑属性、尺度属性和纹理属性四个维度对化学实验物体进行划分。光学属性反映物体在多视角成像中的反射、透射和散射特性，主要包括不透明物体和透明物体，其中不透明物体可进一步表现为漫反射物体和高光反射物体，透明物体则可进一步表现为薄壁透明体、厚壁透明体、半透明体和多界面透明体。拓扑属性反映物体表面连接关系和几何结构形式，主要包括开口实体、闭合实体、薄壁结构和多连通结构。尺度属性反映物体在图像中的相对占比，主要包括常规尺度物体和小尺度物体。纹理属性反映物体表面外观信息的分布特点，主要包括文字纹理、重复纹理、弱纹理和高频纹理。
...
本章围绕化学实验资产重建需求，研究了基于神经隐式场的三维资产重建方法。针对化学实验场景中常见的实验器材、小物体和透明物体等对象，本文以 NeuS 作为基础重建框架，构建了从数据预处理、隐式场训练、网格提取到纹理贴图生成的完整资产重建流程，为后续实验场景生成和仿真平台中的三维资产构建提供支持。

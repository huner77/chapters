...
在面向化学实验的具身智能任务场景中，机器人不仅需要识别实验物体的类别，还需要获得其准确的三维几何与纹理信息，从而支撑后续的抓取、操作、仿真与场景交互。然而，与通用物体相比，化学实验环境中的目标物体具有更强的特殊性：一方面，大量核心物体如烧杯、试管、冷凝管等通常由透明玻璃制成。另一方面，部分实验部件尺寸较小、且局部几何细节丰富。这些因素共同导致传统三维重建算法在化学物体场景下往往难以获得同时兼顾几何精度与纹理质量的重建结果。

现有主流的 RGB-D 深度传感方案通常建立在较理想的表面反射假设之上，但透明玻璃表面普遍存在透射、折射与镜面反射等复杂光学现象，使得深度传感器容易产生大面积深度空洞、噪声与边界失真。近年来神经辐射场技术的发展为解决这些问题提供了新的思路，通过将场景建模为连续的坐标函数，利用体渲染在处理透明材质重建上展现出了显著优势，但在针对化学实验物体的具体应用中，仍面临三个核心挑战：一是对于透明物体，其表面在隐式距离场中未必稳定对应于零等值面，直接沿用传统等值面提取策略，容易出现表面缺失、内部伪影与局部结构错误。二是对于小物体重建，由于其在输入图像中所占像素比例较低，导致训练初期有效监督信号不足，网络容易在过渡态停留过久，从而压缩后续学习态时用于高频细节学习的有效迭代空间。三是神经隐式算法得到的是隐式表征，难以直接满足下游仿真平台、渲染引擎和具身智能系统对于显式网格、标准 UV 坐标与高分辨率纹理贴图的使用需求。

针对上述问题，本章围绕化学实验资产的高保真重建需求，提出了一套基于隐式神经场的重建算法。整体流程如图~\ref{fig:neus_pipeline} 所示，该算法主要包含数据预处理、网络训练、网格提取和纹理贴图生成四个模块，从而形成一条由多视角图像输入到高质量显式三维资产输出的完整重建流程。
...
具体而言，本章首先设计了面向 NeuS 重建的数据预处理流程，通过 COLMAP 恢复相机参数，并结合主体掩模完成稀疏点云去噪与空间归一化，为后续隐式场优化建立稳定的坐标参考。在此基础上，本章把 NeuS 作为隐式重建网络，并针对化学实验物体几何重建中的两个关键问题进行改进：一是在训练阶段，针对小物体训练初期监督信号稀疏、收敛缓慢的问题，提出学习态增强策略，以加速几何初始收敛并为后续细节学习预留更多有效迭代。二是在网格提取阶段，针对透明物体表面与零等值面不稳定对应的问题，采用基于绝对距离场局部极小值搜索的统一表面提取策略，以提高透明区域与不透明区域的几何恢复质量。进一步地，为使隐式重建结果能够直接服务于下游仿真与渲染应用，本章构建了从隐式颜色场到高分辨率显式纹理贴图的自动化生成流程，利用 xatlas 完成 UV 参数化，并结合光栅化烘焙、形态学修复与 KNN（K-Nearest Neighbors\cite{cover1967nearest}, KNN） 补全，生成可直接用于主流仿真平台的高质量纹理资产。

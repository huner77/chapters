...
本章以 NeuS 作为基础训练框架，将目标物体表示为符号距离场和颜色场的组合。其中，符号距离场用于刻画目标物体表面附近的连续几何结构，颜色场用于建模不同空间位置和观察方向下的外观信息。与前文数据预处理阶段得到的主体 Mask 和归一化空间相对应，网络训练阶段需要在目标物体所在区域内优化隐式几何，并使渲染结果与输入图像保持一致。因此，本节不再展开 NeuS 的完整体渲染推导，而是重点说明网络训练目标及其与后续小物体学习态增强策略之间的关系。

从优化目标来看，网络训练主要由颜色重建损失、主体 Mask 约束损失和距离场正则项共同约束。颜色重建损失用于约束网络预测结果与输入图像之间的一致性，使颜色场能够学习目标物体在不同视角下的外观变化。主体 Mask 约束损失用于限制有效重建区域，使网络更加关注目标物体所在空间，减少背景区域对隐式几何学习的干扰。距离场正则项用于约束符号距离场满足局部连续性，使 SDF 梯度能够稳定反映表面附近的几何变化方向。上述训练目标可以概括为
\begin{equation}
    \text{L} =
    \text{L}_{\mathrm{color}}
    + \lambda_{\mathrm{mask}}\text{L}_{\mathrm{mask}}
    + \lambda_{\mathrm{eik}}\text{L}_{\mathrm{eik}},
\end{equation}
其中，$\text{L}_{\mathrm{color}}$ 表示颜色重建损失，$\text{L}_{\mathrm{mask}}$ 表示主体 Mask 约束损失，$\text{L}_{\mathrm{eik}}$ 表示距离场正则项，$\lambda_{\mathrm{mask}}$ 和 $\lambda_{\mathrm{eik}}$ 分别表示对应损失项的权重系数。
